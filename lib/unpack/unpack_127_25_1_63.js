  ...
  if (slice_127_25.byteLength < 10) {
    ...
    for (let i = 0; i < bitmap.length; i++) {
      if (bitmap[i] === 1) {
        ...
        const this_format = formats[subField];
        if (this_format) {
          if (this_format.LenType === 'fixed') {
            ...
            if (!this_format.MaxLen)
              return T.toErrorObject(
                ['max length not implemented for ', this_format.LenType],
                subField
              );
            if (
              this.dataElements[subField] &&
              this.dataElements[subField].length > this_format.MaxLen
            )
